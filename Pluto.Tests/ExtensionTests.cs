...
public class ExtensionTests {
	...
	public void EnsureThatDirectoryExists() {
		var path = Path.Combine(Path.GetTempPath(), "PlutoTest") + Path.DirectorySeparatorChar;
	...
	public void EnsureThatDirectoriesExist() {
		var path1 = Path.Combine(Path.GetTempPath(), "PlutoTest1") + Path.DirectorySeparatorChar;
		if (Directory.Exists(path1)) {
			Assert.Inconclusive();
			return;
		}

		var path2 = Path.Combine(Path.GetTempPath(), "PlutoTest2") + Path.DirectorySeparatorChar;
		if (Directory.Exists(path1)) {
			Assert.Inconclusive();
			return;
		}
